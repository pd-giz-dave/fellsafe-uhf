...
class EventLoop:
    ...
    def call_later_ms(self, delay, callback, *args):
        ...
        id = self.waitq.push(delay, callback, args)
        if isinstance(callback, type_gen):
            prev = callback.pend_throw(id)

    def wait(self, delay):
        # Default wait implementation,
        # to be overridden in subclasses with IO scheduling
        if __debug__ and DEBUG:
            log.debug("Sleeping for: %s mSecs", delay)
        time.sleep_ms(delay)
    ...
    def run_forever(self):
        ...
        while True:
            ...
            while self.waitq.delay(cur_task) == 0:
