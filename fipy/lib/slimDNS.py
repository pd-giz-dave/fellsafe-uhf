""" history
    2021-02-25 DCN: created from https://github.com/nickovs/slimDNS
    2021-02-26 DCN: add logging
    """
""" description
    slimDNS
    ...
    slimDNS is a pure Python implementation of multicast DNS (mDNS) and DNS Service Discovery
    (DNS-SD) targeting memory-constrained devices such as those running MicroPython. The goal
    of this project is to allow MicroPython users to have their networked devices, and the
    services that they offer, more easily discoverable.

    slimDNS directly supports both advertising and discovery of hostnames and it also can be
    used for the advertising and discovery of services. It aims to be compliant with the parts
    of the mDNS and DNS-SD specifications that it implements, even if it fails to implement
    all features.
    """
...
class SlimDNSServer:
    ...
    def advertise_hostname(self, name, find_vacant=True):
        ...
        hostname = check_name(name)
        n = len(hostname)
        if n == 1:
            hostname.append(b"local")
        elif n == 0 or n > 2 or hostname[1] != b'local':
            if __debug__ and log is not None:
                log.error('advertise_hostname: hostname should be a single name component, not {}'.format(name))
            raise ValueError("advertise_hostname: hostname should be a single name component, not {}".format(name))
        ...
        for i in range(MAX_NAME_SEARCH):
            ...
            if not find_vacant or i == MAX_NAME_SEARCH-1:
                if __debug__ and log is not None:
                    log.error('advertise_hostname: Name in use, {}'.format(name))
                raise ValueError("advertise_hostname: Name in use")
        ...
        if __debug__ and log is not None:
            log.info('advertise_hostname: Name {} advertised as {} on {}'.format(name,self.hostname,self.local_addr))
    ...
    def process_packet(self, buf, addr):
        ...
        if not matches:
            if __debug__ and log is not None:
                log.info("process_packet: No matches")
            return
        ...
        if not self._reply_buffer or len(self._reply_buffer) < reply_len:
            if __debug__ and log is not None:
                log.info("process_packet: Making new reply buffer of len {}".format(reply_len))
        ...
        if __debug__ and log is not None:
            log.info("process_packet: Sending packed reply: {}".format(bytes(buf[:o])))
    ...
    def process_waiting_packets(self):
        ...
        while True:
            ...
            if __debug__ and log is not None:
                log.info("process_waiting_packets: Received {} bytes from {} as {}".format(len(buf), addr, buf))
            if buf and addr[0] != self.local_addr:
                try:
                    self.process_packet(memoryview(buf), addr)
                except IndexError as e:
                    if __debug__ and log is not None:
                        log.exc(e,"process_waiting_packets: Index error processing packet; probably malformed data")
                except Exception as e:
                    if __debug__ and log is not None:
                        log.exc(e,"process_waiting_packets: Exception processing packet")
                    # raise e
...
def test():
    ...
    wlan = WLAN(mode=WLAN.AP,ssid='micropython.local',antenna=WLAN.EXT_ANT,channel=1)
    local_addr = wlan.ifconfig(id=1)[0]
    set_debug(True)
